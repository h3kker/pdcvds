...
myteam <- inner_join(team$riders, start_lists %>% select(-team)) %>%
    select(start_date, end_date, race, name, team, number, price, spec, spec_rate) %>%
    mutate(spec = sprintf("%s (%d%%)", spec, spec_rate)) %>%
    select(-spec_rate)

watched <- inner_join(watch, start_lists) %>%
  select(start_date, race, name, team, number) %>%
