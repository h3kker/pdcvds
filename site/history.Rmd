...
season_results <- all_results %>% filter(year(date) == "2023")
```
...
all_results %>%
    ...
    inner_join(all_teams %>% select(pid, season = year), multiple = "all") %>%
    group_by(season) %>%
    arrange(doy) %>%
...
all_results %>%
    ...
    inner_join(all_teams %>% select(pid, name, year), multiple = "all") %>%
...
all_races$results %>%
    ...
    inner_join(all_teams, by = c("name"), multiple="all") %>%
